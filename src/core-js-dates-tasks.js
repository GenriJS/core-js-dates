...
function getNextFriday(date) {
  ...
  const delta = 5 - dayNumber;
  if (dayNumber === 5) {
    const nextFriday = new Date(Date.parse(currentDate) + 86400000 * 7);
    return nextFriday;
  }
  if (dayNumber === 6) {
    const nextFriday = new Date(Date.parse(currentDate) + 86400000 * 6);
    return nextFriday;
  }
  const nextFriday = new Date(Date.parse(currentDate) + 86400000 * delta);
  return nextFriday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const delta = (endMs - startMs) / 86400000;
...
function getNextFridayThe13th(date) {
  ...
  const nextFriday = new Date(dateObj.setDate(13));
  if (dateObj.getDate() < 13 && nextFriday.getDay() === 5) return nextFriday;
